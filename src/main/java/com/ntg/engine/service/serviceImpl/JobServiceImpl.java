...
public class JobServiceImpl implements JobService {
    ...
    public boolean scheduleOneTimeJob(String jobName, Class<? extends QuartzJobBean> jobClass, Date date,
                                      Schedule schedule, Map<String, Object> tenantInfo) {
        ...
        JobDetail jobDetail = JobUtil.createJob(jobClass, false, context, jobKey, groupKey, schedule, tenantInfo);
    ...
    public boolean isJobWithNamePresent(String jobName) {
        try {
            ...
            if (scheduler.checkExists(jobKey)) {
                System.out.println("Job " + jobName + " running since " + scheduler.getJobDetail(jobKey).getJobDataMap().get("startedAt"));
                return true;
            }
        } catch (SchedulerException e) {
